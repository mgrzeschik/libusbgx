...
static int uvc_libconfig_import(struct usbg_function *f, config_setting_t *root)
{
	...
	ret = uvc_set_class(uvcf, "control");
	if (ret != USBG_SUCCESS)
		return ret;

	ret = uvc_set_class(uvcf, "streaming");
	if (ret != USBG_SUCCESS)
		return ret;
...
static int uvc_set_frame(char *format_path, const char *format, const struct usbg_f_uvc_frame_attrs *attrs)
{
	...
	nmb = snprintf(frame_name, sizeof(frame_name), "%dp%d", attrs->wHeight, attrs->bFrameIndex);
...
int usbg_f_uvc_set_attrs(usbg_f_uvc *uvcf, const struct usbg_f_uvc_attrs *attrs)
{
	...
	for(format_attrs = attrs->formats, j = 0; format_attrs[j]; ++j) {
		ret = uvc_set_streaming(path, format_attrs[j]->format, format_attrs[i]);
		if(ret != USBG_SUCCESS)
			ERROR("Error: %d", ret);
	}
